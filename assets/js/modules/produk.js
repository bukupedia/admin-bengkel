...
// INIT
export function initProdukPage() {
  ...
  if (filteredData.length === 0) {
    table.innerHTML = `<tr><td colspan="4" class="text-center py-4">
      <div class="text-muted">
        <p class="mb-1">⚙️</p>
        <p>${searchQuery ? "Tidak ada hasil pencarian" : "Belum ada data produk"}</p>
        <small>${searchQuery ? "Coba kata kunci lain" : "Klik tombol 'Tambah' untuk menambahkan produk"}</small>
      </div>
    </td></tr>`;
    return;
  }
...
function editPart(id) {
  ...
  // Update modal title
  document.querySelector("#modalPart .modal-header h5").textContent = "Edit Produk";
...
function clearForm() {
  ...
  // Reset modal title
  document.querySelector("#modalPart .modal-header h5").textContent = "Tambah Produk";
}
